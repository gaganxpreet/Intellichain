...
const LogisticsOptimizer: React.FC = () => {
  ...
  // Update route data when real driving data is available from Google Maps
  const handleRouteDataUpdate = React.useCallback((distanceKm: number, durationMin?: number) => {
    if (distanceKm > 0 && result) {
      ...
      const duration = toNumber(durationMin || result.totalTime);
      const discount = toNumber(result.poolingDiscount || 0);
      
      const baseCost = km * ratePerKm;
      const finalCost = baseCost * (1 - discount);
      const originalCost = discount > 0 ? baseCost : undefined;
      const savings = originalCost ? originalCost - finalCost : undefined;
      
      setRouteData({
        distance: round2(km),
        duration: Math.round(duration),
        cost: round2(finalCost),
        originalCost: originalCost ? round2(originalCost) : undefined,
        savings: savings ? round2(savings) : undefined,
        poolingDiscount: discount
      });
    }
  }, [result]);
  ...
  // Initialize route data from algorithm result
  React.useEffect(() => {
    if (result && !routeData) {
      // Use algorithm data as initial fallback
      setRouteData({
        distance: round2(toNumber(result.totalDistance)),
        duration: Math.round(toNumber(result.totalTime)),
        cost: round2(toNumber(result.totalCost)),
        originalCost: result.originalCost ? round2(toNumber(result.originalCost)) : undefined,
        savings: result.savings ? round2(toNumber(result.savings)) : undefined,
        poolingDiscount: toNumber(result.poolingDiscount || 0)
      });
    }
  }, [result, routeData]);
  ...
  const handleOptimize = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    setRouteData(null); // Reset route data for new calculation
                    ...
                    <div className="grid md:grid-cols-3 gap-6 mb-8">
                      ...
                      <div className="text-center">
                        <div className="flex items-center justify-center mb-2">
                          ...
                          <span className="text-3xl font-bold text-white">
                            {routeData?.distance || result.totalDistance}
                          </span>
                        </div>
                      ...
                      <div className="text-center">
                        <div className="flex items-center justify-center mb-2">
                          ...
                          <span className="text-3xl font-bold text-white">
                            {routeData?.duration || result.totalTime}
                          </span>
                        </div>
                    ...
                    <div className="mt-6 p-4 bg-green-500/20 rounded-xl border border-green-500/30">
                      ...
                      {routeData && (
                        <p className="text-green-200 text-sm mt-2">
                          Route data updated with real driving distance from Google Maps
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-white/10 backdrop-blur-lg rounded-3xl border border-white/20 overflow-hidden shadow-2xl">
                    ...
                    <div className="h-96">
                      <GoogleMapVisualization
                        ...
                        hub={result.hub}
                        result={result}
                        onDistanceUpdate={handleRouteDataUpdate}
                      />
                    </div>
                  </div>
